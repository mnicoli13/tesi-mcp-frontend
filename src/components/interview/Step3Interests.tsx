...
const Step3Interests: React.FC<Step3InterestsProps> = ({
  ...
  const handleNextStep = async () => {
    const isValid = await interestsDataSchema.isValid(formValues);
    if (!isValid) return;
    onSave(formValues);
    handleNext();
  };
      ...
      <CardContent sx={{ p: { xs: 2, md: 4 }, minHeight: 400 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary" mb={3}>
            Aiutaci a capire le tue preferenze lavorative. Queste informazioni
            ci permetteranno di suggerirti opportunità di carriera in linea con
            i tuoi interessi.
          </Typography>

          <Alert severity="info" sx={{ mb: 4 }} icon={<InterestsIcon />}>
            Questi dati saranno utilizzati dai nostri strumenti di AI per
            personalizzare i suggerimenti di carriera e filtrare le opportunità
          ...
          <Stack spacing={4}>
            ...
            >
              ...
              <Typography variant="body2" color="text.secondary" mb={2}>
                Seleziona uno o più tipi di azienda in cui ti piacerebbe
                lavorare
              </Typography>
              <Controller
                name="companyType"
                ...
                render={({ field }) => (
                  <FormGroup>
                    <Grid container spacing={2}>
                      {Object.entries(COMPANY_TYPE_LABELS).map(
                        ([value, label]) => (
                ...
                render={({ field }) => (
                  <FormControl component="fieldset" error={!!errors.workStyle}>
                    <RadioGroup {...field} value={field.value}>
                      <Grid container spacing={2}>
                        {Object.entries(WORK_STYLE_LABELS).map(
                          ([value, label]) => (
