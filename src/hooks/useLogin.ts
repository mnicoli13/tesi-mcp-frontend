      ...
      if (success) {
        ...
        if (userStr) {
          try {
            ...
            if (userId) {
              // Importa interviewService dinamicamente per evitare dipendenze circolari
              const { interviewService } = await import('../services/interviewService');
              
              try {
                const isInterviewCompleted = await interviewService.checkInterviewCompletion();
                ...
                console.error('Errore nel verificare completamento intervista:', error);
                navigate('/interview', { replace: true });
              }
            } else {
